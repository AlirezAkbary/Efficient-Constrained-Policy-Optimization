...
def run_GDA_agent(cmdp, learning_rate_pol, learning_rate_lambd, num_iterations, ub_lambd,
    ...
    for t in range(num_iterations):
        ...
        if update_dual_variable:
            ...
            dual.update(v_c)
            cv = cmdp.b - v_c
            cv_list.append(cv)  # calculates cv with true V_g
            V_c_list.append(v_c)
        else:
...
if __name__ == '__main__':
    ...
    ret, cv = run_GDA_agent(**run_agent_params)
    current_dir = os.path.join(os.getcwd(), "Results/GDA/")
