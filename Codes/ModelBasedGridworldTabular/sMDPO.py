...
def run_agent(cmdp, optimal_performance, num_iterations, eta, alpha_policy, m, ub_lambd, initial_theta,
                 initial_policy, alpha_lambd, update_dual_variable=True):
    """
    Parameter:
    ...
    for t in range(num_iterations):
        ...
        J_c = np.zeros(cmdp.num_constraints)
        ...
        primal.update_policy_param(curernt_policy_prob, cmdp.p0, A_l)
...
if __name__ == '__main__':
    ...
    current_dir = os.path.join(os.getcwd(), "Results/sMDPO_ModelBased/")
    param_dir = "R" + str(args.run) + "_iter" + str(args.num_iterations) + "_alphalam" + str(args.alpha_lambd)\
        + "_m" + str(args.m) + "_eta" + str(args.eta) + "_alphapol" + str(args.alpha_policy) + "_gamma" + str(args.gamma)
