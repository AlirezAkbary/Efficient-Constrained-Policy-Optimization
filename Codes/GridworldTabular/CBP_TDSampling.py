...
"""
Coin Betting implementation for parameter update of simplex (policy) and d-dim vector (lambda, dual variable) with TD Sampling
"""
...
def run_CB_agent(cmdp, optimal_performance, num_iterations, ub_lambd, moving_avg_window,
                 full_average, initial_policy, alpha_lambd, nsamples, update_dual_variable=True):
    ...
    for t in range(num_iterations):
        ...
        Q_l_hat = Q_hat[0]
        V_r_hat_list.append(np.dot(cmdp.p0, np.einsum('sa,sa->s', curr_policy.policy_prob, Q_l_hat)))
        ...
        primal.update(Q_l_hat / grad_pol_max)
        ...
        print("optimality gap", og_list[-1])
        print("constrain violation", cv_list[-1])
...
if __name__ == '__main__':
    ...
    #     int(args.multiple_constraints)) + "_FullAvg" + str(int(args.full_average)) \
    #                   + "_nsamples" + str(args.num_samples) + "_gam" + str(args.gamma) + "_b" + str(args.b_thresh)

    # fold_name = os.path.join(save_dir_loc, "Results/Tabular/CBP/ModelFree/TDSampling")
    ...
    current_dir = os.path.join(os.getcwd(), "Results/CBP_TDSampling/")
