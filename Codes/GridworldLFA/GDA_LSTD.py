...
"""
GDA: Gradient Descent and Ascent. This implements a LFA version with LSTD based sampling to estimate the Q_hat function.
"""
...
def run_GDA_agent(cmdp, optimal_performance, learning_rate_pol, learning_rate_lambd,
                  ...
                  initial_policy, num_samples,  feature, tc_args, update_dual_variable=True):
    ...
    # save_data(output_dir, np.asarray(og_list), np.asarray(cv_list), np.asarray(avg_og_list), np.asarray(avg_cv_list),
    #           np.asarray(curr_policy.policy_prob), np.asarray(lambd_list), np.asarray(avg_lambd_list),
    #           np.asarray(policy_list), np.asarray(Q_l_list))
    # store_noisy_estimator_data(output_dir, diff_in_Q_r, diff_in_Q_c, diff_in_Q_l, V_g_hat_list, V_r_hat_list,
    #                            V_g_list, V_r_list, Q_l_hat_list)
...
if __name__ == '__main__':
    ...
    print("Gridworld LFA with GDA using args:")
    print(args)
    if try_locally:
        save_dir_loc = "./"
    else:
    ...
    # fold_name = os.path.join(save_dir_loc, "Results/Tabular/GDA/ModelFree/LFA/TileCodingFeatures")
    ...
    current_dir = os.path.join(os.getcwd(), "Results/GDA_LSTDSampling/")
    param_dir = "R" + str(args.run) + "_iter" + str(args.num_iterations) + "_alphalam" + str(args.learning_rate_lambd)\
        + "_alphapol" + str(args.learning_rate_pol) + "_Qnsamples" + str(args.num_samples)\
    ...
    og, cv, avg_og, avg_cv = run_GDA_agent(**run_agent_params)
